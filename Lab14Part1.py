...
def run():
    ...
    # ——— Data loading & one‑hot as before ———
    ...
    # —— He initialization for every weight matrix —— 
    weights = []
    biases  = []
    ...
    for epoch in range(epochs):
        ...
        for xi, yi in zip(x1, y1):
            # —— forward pass with ReLU hidden, sigmoid output —— 
            ...
            # propagate through hidden with ReLU derivative
            for l in range(len(weights)-1, 0, -1):
                d = deltas[-1].dot(weights[l].T) * relu_deriv(activations[l])
                deltas.append(d)
            deltas.reverse()  # so deltas[i] corresponds to layer i→i+1
...
def relu_deriv(x):
    # x is activation → derivative is 1 where x>0
